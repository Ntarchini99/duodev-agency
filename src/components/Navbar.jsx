...
const Header = () => {
  ...
  return (
    <header className={`flex items-center justify-between xl:justify-start w-full py-4 px-8 h-[10vh] z-50 fixed top-0 ${showMenu ? 'bg-white' : ''}`}>
      ...
      <nav className={`w-[80%] md:w-[40%] xl:w-full ${showMenu ? "h-full" : ""} xl:static flex-1 flex flex-col xl:flex-row items-center justify-center transition-all duration-500 z-50 ${showMenu ? "absolute top-16 left-0 bg-white" : "hidden xl:flex"}`}>
